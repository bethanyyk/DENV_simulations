...
ISOLATES = [i for i in open("results/DENV3/unique_isolates.txt").read().split('\n') if len(i) >0]

# Generate 4 random proportion combinations
isolate_combinations = list(itertools.combinations(ISOLATES, 2))
primer_bed_file = "results/DENV3/DENV3.bed"
for isolate_combination in isolate_combinations:
    proportion1 = round(random.uniform(0, 1), 2)
    proportion2 = round(1.0 - proportion1, 2)
    file1_path = "results/DENV3/" + isolate_combination[0] + ".fasta"
    file2_path = "results/DENV3/" + isolate_combination[1] + ".fasta"
    output_path = "lineage_combine/" + isolate_combination[0] + "_" + str(proportion1) + "_" +  isolate_combination[1] + "_" + str(proportion2)
    command = [
